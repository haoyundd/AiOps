    ...
    async def event_generator():
        try:
            async for chunk in rag_agent_service.query_stream(request.question, session_id=request.id):
                ...
                if chunk_type == "debug":
                    # 调试信息，可以选择发送或忽略
                    yield {#return → 返回数据，函数结束。yield → 吐出数据，函数继续跑
                        "event": "message",
                        "data": json.dumps({#json.dumps把字典变成json字符串
                            "type": "debug",
                            "node": chunk.get("node", "unknown"),
                            "message_type": chunk.get("message_type", "unknown")
                        }, ensure_ascii=False)#ensure_ascii=False让它支持中文，不乱码。
                    }
                    #工具调用信息	{"tool": "retrieve_knowledge", "status": "start"}
