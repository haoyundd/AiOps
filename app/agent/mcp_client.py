...
# 全局 MCP 客户端（延迟初始化）先占个坑，现在还没创建客户端，只是声明一下：以后这里要放全局唯一的 MCP 客户端。
_mcp_client: Optional[MultiServerMCPClient] = None


#提前写好一套规则：以后凡是调用 MCP 工具失败，自动重试 3 次。现在只是定义函数，还没运行。
...
# 从配置文件读取 MCP 服务器配置把配置组装成 MCP 服务器字典
#把配置里的两个地址，拼成框架能识别的格式：
...
async def get_mcp_client(
        #option指定类型。servers 可以是 Dict 类型，也可以是 None（默认值）
    ...
    if force_new:
        ...
        client = _create_mcp_client(
            # DEFAULT_MCP_SERVERS是我们在 app/config.py 里定义的 MCP 服务器配置
...
async def get_mcp_client_with_retry(
    ...
    return await get_mcp_client(# 调用 get_mcp_client 函数。然后去上面的 get_mcp_client 函数里看。
        servers=servers,
