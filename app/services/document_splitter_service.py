...
class DocumentSplitterService:
    ...
    def __init__(self):
        ...
        self. text_splitter = RecursiveCharacterTextSplitter(
            ...
            chunk_overlap=self.chunk_overlap,#（分片间重叠，保持上下文连续性）
            length_function=len,
...
#List[Document]，一个 Document 对象组成的列表。 每个 Document 包含原始文本、元数据（如文件路径、扩展名、文件名）。
#每个Document就是一个包裹，里面两样东西：
    # doc.page_content 是文本内容，给模型读的。
    # doc.metadata  # 元数据字典，记录来源文件名、标题等
