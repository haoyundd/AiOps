...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            dir_path = Path(target_path).resolve()# 转换为绝对路径

            if not dir_path.exists() or not dir_path.is_dir():
                raise ValueError(f"目录不存在或不是有效目录: {target_path}")

            result.directory_path = str(dir_path)# 记录索引目录路径
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            # 1. 读取文件内容
            content = path.read_text(encoding="utf-8")#将文件内容作为字符串返回
            logger.info(f"读取文件: {path}, 内容长度: {len(content)} 字符")

            # 2. 删除该文件的旧数据（如果存在）
            normalized_path = path.as_posix()# 转换为 POSIX 路径（统一使用 / 作为分隔符）
            vector_store_manager.delete_by_source(normalized_path)# 删除这个路径的旧数据
